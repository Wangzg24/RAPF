    ...
    def forward(self, inputs, cat=True):
        if not self.cat_entity_rep:
            ...
            if cat:
                ...
                sequence_outputs = outputs[0]  # [20, 128, 768]
                tensor_range = torch.arange(inputs['word'].size()[0])  # inputs['word'].shape  [20, 128]
                h_state = outputs[0][tensor_range, inputs["pos1"]]  # h_state.shape [20, 768]
                t_state = outputs[0][tensor_range, inputs["pos2"]]  # [20, 768]
                ...
                # return h_state, t_state, outputs['last_hidden_state'], outputs['pooler_output']
                return h_state, t_state, outputs[0], outputs[1]
            else:
