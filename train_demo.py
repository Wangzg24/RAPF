...
def main(seed):
    ...
    if encoder_name == 'cnn':
        try:
            print("use cnn for encoder!")
            glove_mat = np.load('./pretrain/glove.6B.50d_mat.npy')
            glove_word2id = json.load(open('./pretrain/glove.6B.50d_mat_word2id.json'))
        except:
    ...
    if opt.pair:
        ...
        train_data_loader = get_loader(opt.train, sentence_encoder,
                                       N=trainN, K=K, Q=Q, na_rate=opt.na_rate, batch_size=batch_size, flag="train")
        val_data_loader = get_loader(opt.val, sentence_encoder,
                                     N=N, K=K, Q=Q, na_rate=opt.na_rate, batch_size=batch_size, flag="eval")
        test_data_loader = get_loader(opt.test, sentence_encoder,
                                      N=N, K=K, Q=Q, na_rate=opt.na_rate, batch_size=batch_size, flag="test")
        if opt.adv:
    ...
    if not opt.only_test and (not opt.test_online):
        ...
        if ckpt is None:
            ...
            ckpt = 'none'
        framework.test_1(model, batch_size, N, K, Q, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair,
