...
class BayesianDynamicBoundaryPrototypicalNetwork(nn.Module):
    def __init__(self, encoder_dim, num_classes, prior_strength=1.0):
        ...
        self.cov_adapter = nn.Sequential(
            nn.Linear(encoder_dim, int(512 / 2)),
            nn.ReLU(),
            nn.Linear(int(512 / 2), encoder_dim),
            nn.Softplus()
        )

        # ====== 注册常量缓冲区 ======
        self.register_buffer('eye_matrix', torch.eye(encoder_dim))
        # self.register_buffer('eye_matrix', torch.eye(768))

        # 变换维度
        # self.bh = nn.Linear(512, 512 / 2)
    ...
    def compute_bayesian_covariance(self, support_embeddings):
        ...
        # 样本统计量（K=1）
        sample_covs = self.eye_matrix.to(support_embeddings.device) * 0.01  # [D, D] 初始化较小的方差 0.01
        sample_covs = sample_covs.unsqueeze(0).unsqueeze(0).expand(B, N, -1, -1)
        ...
        #修改维度
        # prior_means = self.bh(prior_means)
        ...
        # posterior_covs = self.ensure_positive_definite(posterior_covs)[:, :, :768, :768]


        # 改变维度
        # posterior_means = self.bh(posterior_means)

        # posterior_covs = self.bh(posterior_covs)
