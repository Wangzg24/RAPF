from fewshot_re_kit.data_loader import get_loader, get_loader_pair, get_loader_unsupervised, get_loader_pair2, \
    get_loader2
...
def main(seed):
    ...
    elif model_name == 'xiangguanxing':
        model = XIANGGUANXING(sentence_encoder, dot=opt.dot, relation_encoder=relation_encoder)

    elif model_name == 'pinjiechaxun':
        model = PINJIECHAXUN(sentence_encoder, dot=opt.dot, relation_encoder=relation_encoder)

    elif model_name == 'gnn_get':
        model = GnnGet(sentence_encoder, dot=opt.dot, relation_encoder=relation_encoder)

    elif model_name == 'distillation':
        model = ZhengLiu(sentence_encoder, dot=opt.dot, relation_encoder=relation_encoder)

    elif model_name == 'vae':
        model = VAE(sentence_encoder, dot=opt.dot, relation_encoder=relation_encoder)
    ...
    if not opt.only_test and (not opt.test_online):
        ...
        if ckpt is None:
            ...
            ckpt = 'none'
        framework.test(model, batch_size, N, K, Q, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair,
